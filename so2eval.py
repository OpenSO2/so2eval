...
Examples:
datepattern = .*(?P<year>\w{4})_(?P<month>\w{2})_(?P<day>\w{2})-
(?P<hour>\w{2})_(?P<minute>\w{2})_(?P<second>\w{2})_(?P<millisecond>\w{3})
testing_2017_06_08-12_19_44_091_cam_bot.png

datepattern = .*(?P<year>\w{4})(?P<month>\w{2})(?P<day>\w{2})_(?P<hour>\w{2})
(?P<minute>\w{2})(?P<second>\w{2})_(?P<millisecond>\w{3})
...
options = {
	...
	"use_bg_correction": {"default": True, "type": str,
		"help": "Flag if background correction should be done."
		"If set, plume_free_field must also be set."},
	"roi": {"default": '[0, -1, 0, -1]', "type": str,
		"help": "The region of interest in which the files are evaluated."
		"The rest will be cropped. Form: [top, bot, left, right] in px."},
	"plume_free_field": {"default": '[340, 620, 1088, 1300]', "type": str,
		"help": "If background correction is used, this part of the images will"
		"be used for evaluation. Relative to the roi."
		"Form [top, bot, left, right] in px."},
...
parser = argparse.ArgumentParser(
	description='Preprocess SO2 images to ppm*m numpy files')
parser.set_defaults(**defaults)

for arg in defaults:
	parser.add_argument('--' + arg, dest=arg, type=options[arg]["type"],
		help=options[arg]["help"] + " Default: " + str(options[arg]["default"]),
	)
